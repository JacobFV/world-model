"""FRED/ALFRED evidence emission for ``fred_county_laus_monthly_vintages``.

Copied from ``data/fred_county_vintages/alfred.py`` so the two datasets emit the *same record
contract* -- ``attributes.realtime_start``, ``dimensions.vintage``, ``dimensions.first_release``,
subjects ``geo:US:county:<SSCCC>`` -- and ``worldmodel.embedding.realtime_panel`` reads either one
unchanged. What that file already did and this one keeps:

* Subjects are county-equivalents, ``geo:US:county:<SSCCC>``, with the code ``build_config.py`` took
  from published geography. The county entity sits ``within`` its state, ``within`` the country.
* Series metadata is per family (``config.json``'s ``families``), not per series: 6,282 series share
  two families, so a series carries only ``[family, fips, fips_source]``.
...
* ``vintage_units`` reads the units FRED published *in that vintage*, which this dataset needs as
  much as the annual one needed it for rebased real GDP: county labor force is "Thousands of Persons"
  before 2016-03-18 and "Persons" after, and the observations changed with the label.
* The fredgraph.csv reader (current vintage only) is absent: a current-vintage code path would emit
  rows that are not real time.

Two differences from the annual file, both because of what is in this dataset:

* ``period`` here resolves reference periods at ``M``: a row dated ``2008-09-01`` is the half-open
  month ``[2008-09-01, 2008-10-01)``.
* There is no ``bea_combination_area`` code scheme: every series here is BLS LAUS and every code is a
  Census county FIPS, so ``code_scheme`` is always ``fips``.
"""
...
def code_scheme(family, fips):
    """Every code here is a Census county FIPS: BLS LAUS publishes no combination areas at county level.

    Kept as a function, and emitted on every record, so a consumer can read this dataset and
    ``fred_county_vintages`` (where BEA's combination areas do need marking) through one field.
    """
    return 'fips'
