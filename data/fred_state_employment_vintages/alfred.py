"""FRED/ALFRED evidence emission for ``fred_state_employment_vintages``.

Derived from ``data/fred_macro_panel/fred_alfred.py``, which the six ``fred_*`` anchor datasets
keep as byte-identical copies (a test there checks it). This copy is deliberately **not**
byte-identical: state x supersector employment needs an industry dimension, which the national
macro panel has no use for. The differences are exactly:

* :meth:`Emitter.observation` adds ``dimensions.industry``, ``dimensions.ces_industry_code`` and
  ``dimensions.panel_role``, so a consumer selects one supersector without parsing series ids.
* The ``economic_series`` entity carries the same fields as attributes.
* The fredgraph.csv reader (current vintage only) is removed. This dataset exists to carry
  vintages; a current-vintage-only code path would silently emit rows that are not real time,
  which is the exact failure it was built to fix.

Full artifacts are FRED API ``series/observations`` JSON shards (one per series x real-time
window) whose request parameters name ``series_id``/``realtime_start``/``realtime_end``; every
observation carries its ALFRED real-time period.
"""
...
class Emitter:
    ...
    def context_records(self, series_id, evidence, observed_at):
        ...
        created = self.entity(series_key, 'economic_series', meta.get('title') or series_id, evidence, observed_at,
                              ...
                              industry=meta.get('industry'), ces_industry_code=meta.get('ces_industry_code'),
                              panel_role=meta.get('panel_role'), state=meta.get('state'))
        if created:
            out += created
    ...
    def observation(self, series_id, row, evidence, *, realtime_start, realtime_end, clipped, retrieved_at, tier):
        ...
        record = {'kind': 'observation', 'id': f'fred:obs:{series_id}:{row["date"]}:{realtime_start}',
                  ...
                  'dimensions': {'series_id': series_id, 'frequency': meta.get('frequency'),
                                 ...
                                 # Industry on the observation: a consumer selects one CES supersector
                                 # without parsing series ids or titles.
                                 'industry': meta.get('industry'), 'ces_industry_code': meta.get('ces_industry_code'),
