...
Market Yield on U.S. Treasury Securities at 10-Year Constant Maturity (`DGS10`), daily, percent unless noted, from FRED/ALFRED.
...
- Full acquisition (`wm acquire fred_treasury10y --allow-network`): FRED API `series/observations` for `DGS10` over the whole
...
- Sample (`wm sample fred_treasury10y`): the dated 2025 Q1 fredgraph.csv excerpt, unchanged.
- The last window grows by ~250 vintage dates a year; when it approaches 2,000 split it (see
  `wm acquire fred_treasury10y --dry-run` and FRED `series/vintagedates`).

## Evidence

`normalized` (gzip evidence JSONL):
- entities `geo:US` (country) and `fred:DGS10` (economic_series) plus assertion `fred:DGS10 describes_location geo:US`;
- observations `subject=geo:US`, `metric=treasury_yield_10y`, `unit=percent`, `valid_from`/`valid_to` = the observation day
...
```sh
wm acquire fred_treasury10y --dry-run
wm acquire fred_treasury10y --allow-network
WORLD_MODEL_RAW_VERIFY=size wm run fred_treasury10y
wm verify fred_treasury10y
```
