"""Dataset-local, bounded-memory evidence emission (copied per dataset; stdlib only).

Record IDs are deterministic digests of (dataset, raw artifact, identity). Entities
are emitted once per entity_id across the whole stage via a spill-to-SQLite key set,
so memory stays bounded even for millions of distinct keys.
"""
import math
import os
import sqlite3
...
def num(value):
    """Parse a numeric source cell; None for blanks/missing markers. Integers stay ints."""
    if value is None:
        return None
    if isinstance(value, (int, float)) and not isinstance(value, bool):
        ...
        try:
            ...
            return None  # machine-generated files contain occasional corrupt numeric cells (e.g. '42#.5')
...
class Evidence:
    """Builds validated-shape evidence records bound to one raw input."""
    ...
    def _base(self, kind, identity, locator, attrs=None, **fields):
        record = {'kind': kind, 'id': self.prefix + ':' + digest([self.dataset, self.ref, identity]),
                  'observed_at': self.observed_at, 'evidence': self.context.raw_evidence(locator, self.index),
    ...
    def relation(self, subject, predicate, obj, locator, identity=None, once=True, **attrs):
        ...
        ident = ['rel', subject, predicate, obj] + ([identity] if identity is not None else [])
        if once and not self.keys.add('r|' + digest(ident)):
            return None
    ...
    def claim(self, subject, predicate, value, locator, identity=None, **attrs):
        ident = ['claim', subject, predicate, identity if identity is not None else value]
    ...
    def observation(self, subject, metric, value, unit, locator, *, valid_from=None, valid_to=None,
        ...
        ident = ['obs', subject, metric, valid_from, valid_to, dims] + ([identity] if identity is not None else [])
    ...
    def event(self, event_type, occurred_at, participants, locator, identity, **fields):
        ...
        return self._base('event', ['event', identity], locator, attrs, event_type=event_type,
                          occurred_at=occurred_at, participants=list(participants), **fields)
