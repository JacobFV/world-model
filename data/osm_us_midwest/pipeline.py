"""OpenStreetMap network topology for the Geofabrik US Midwest extract (osm_us_midwest).
...
def run(context):
    if not context.raw_inputs or not _full(context):
        raise ValueError('osm_us_midwest expects a sharded Geofabrik .osm.pbf acquisition (wm acquire osm_us_midwest)')
