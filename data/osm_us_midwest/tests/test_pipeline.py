...
def _helpers():
    spec = importlib.util.spec_from_file_location('osm_us_midwest_helpers_test', HERE / 'helpers.py')
...
class OsmTopologyPipelineTests(unittest.TestCase):
    ...
    def run_full(self, workers):
        ...
        raw = self.store.import_shards('osm_us_midwest', [shard], {'publisher': 'fixture', 'acquisition': {'reader': None}},
                                       complete=True, method='link')
        runner = Runner(Catalog(PROJECT / 'data'), self.store, PROJECT)
        ref = runner.run('osm_us_midwest', parameters={'workers': workers}, raw_refs={'osm_us_midwest': [raw]})
        return raw, ref
    ...
    def test_sample_payload_is_rejected(self):
        ...
        path.write_text('{}\n')
        raw = self.store.import_file('osm_us_midwest', path, source={'publisher': 'fixture'})
        with self.assertRaisesRegex(Exception, 'sharded Geofabrik'):
            Runner(Catalog(PROJECT / 'data'), self.store, PROJECT).run('osm_us_midwest', raw_refs={'osm_us_midwest': [raw]})
