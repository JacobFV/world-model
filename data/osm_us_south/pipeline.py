"""OpenStreetMap network topology for the Geofabrik US South extract (osm_us_south).
...
def run(context):
    if not context.raw_inputs or not _full(context):
        raise ValueError('osm_us_south expects a sharded Geofabrik .osm.pbf acquisition (wm acquire osm_us_south)')
