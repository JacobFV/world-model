...
| Embed a dated subgraph as a state vector | The root-readout encoder ([world-state embeddings](world-state-embeddings.md)) embeds any as-of subgraph; `wm embed-query` ranks the places whose past state is nearest to one county's state now | Measured over eight scored attempts in five domains: it beats the naive baseline everywhere and a gradient-boosted model on the same subgraph only on county employment (pooled test; not significant year-clustered), and adds nothing to that model when stacked into it. On a first-release panel, where no revision leakage remains, it loses to drift and to that model on county population. Nothing it produces is validated, and after a link-prediction attempt with an empty query node also lost to that model, the pre-declared stop rule closed the encoder line; the interval head is what continues |
...
| --- | --- | --- |
| Regional demand / market entry | ACS 5-year tables (4.5M), ACS PUMS, Census PEP (2.4M), CBP/economic census (7.9M), BEA regional (13.7M), IRS SOI migration (2.7M), LODES (3.5M), and now 5.25M **vintaged** county observations (fred_county_vintages) with the first-release panel built on them | No local price or consumption panel; no firm-level demand; no validated demand model. Geography vintages must be crosswalked per year |
