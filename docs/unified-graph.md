...
| --- | --- | --- |
| a sanctioned legal entity that is also an SEC filer | sanctions datasets publish **1,928** distinct LEI values (1,927 present in GLEIF). **Zero** carry an SEC registration authority, and that is still true after the 2026-09-19 rebuild. By jurisdiction: RU 546, US-DE 145, CN 137, CY 92, DE 73, GB 63 …; the 241 US ones carry Delaware and other state file numbers (`RA000602` 139), not CIKs | **different populations.** A US entity on a sanctions or ownership list is a state-registered company, not an SEC registrant. No bridge can create this |
| an actually *designated* party whose corporate group reaches an SEC filer | OFAC SDN and the other designation lists publish **183** distinct LEIs. Zero are SEC registrants by GLEIF's own registration-authority field, their whole published ownership neighbourhood within two GLEIF/OpenSanctions hops is **122** further LEIs, and **zero of those** are SEC registrants either. **Updated 2026-09-19:** on the rebuilt index 99 asserted-identity clusters carry a listing, an LEI and a CIK together, but **1** of the 99 is a designation and **all 99** hold together only because a `wikidata:` member bridges the LEI to the CIK | **still absent as a published fact.** One cluster is not a population, the bridge is a community assertion rather than a registration authority, and the other 98 anchor on ownership or politically-exposed-person records. q1 prints both breakdowns for exactly this reason |
...
| `market_corporate_actions` ticker↔CIK↔FIGI | the dataset republished **after** that index was built, so its edges were not in it at all. **Measured again after the 2026-09-19 rebuild:** it is indexed now — 2,226,975 records, and the index carries 40,587 `issuer_listing`, 16,659 `listing_security` and 13,182 `primary_listing` edges — and identity still does not move: **0** asserted-identity clusters contain a `ticker:` entity, of 56,223 in the index (15,986 of them `ticker:US:`), and there are **0** `figi:` entities to join to. Separately, `ticker:US:<T>` (market_prices 15,983, alpaca 11,594) shares **0** entity IDs with `ticker:<MIC>:<T>` (sec_issuer_reference 9,859, nasdaq_listings 13,199), which its own `primary_listing` edges are what would join | **done, and it confirms the reading.** The rebuild put the edges in the graph and changed nothing about identity, which is correct: a ticker is a listing, never issuer identity, so it belongs in the graph as edges and `MAPPING_SPECS` keeps `sec_cik_ticker` as `listed_as` |
