...
def execute(args,catalog,store,project,reference):
    ...
    if args.command=='environment':
        from .environments import Environment,TemporalEvaluator
        ...
        adapter=TemporalEvaluator(store,graph,request['materialization'],request['step_seconds'],
        ...
        frames=[{'observation':observation,'info':info,'reward':None}];inputs=[graph,env.materialization['artifact']]
        ...
        for action in ([] if info.get('terminated') or info.get('truncated') else request['actions']):
            ...
            inputs.append(env.materialization['artifact'])
            if terminated or truncated:break
        inputs=list({canonical(ref):ref for ref in inputs}.values())
        result={'frames':frames,'input_history':env.history,'materialization_ref':env.materialization['artifact'],
                'snapshots':env.materialization['snapshots'],'replay_process_calls':adapter.total_calls,
                ...
                'limitations':['Rewards reflect explicit objectives, not validated real-world utility.',
                               'Full-prefix replay preserves process history but is not a high-throughput RL trainer.',
        ...
        return {'artifact':artifact,'frames':frames,'replay_process_calls':adapter.total_calls}
