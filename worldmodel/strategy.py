...
def evaluate_strategies(config):
    ...
    if ranking_mode not in ('expected','worst_case','minimax_regret'):
    ...
    ranking=[]
    for policy in policies:
        rows=[row for row in outcomes if row['policy']==policy['id']]
        ...
        feasible=not any(row['violations'] for row in rows)
        score={'expected':sign*expected,'worst_case':sign*worst,'minimax_regret':-max(regrets)}[ranking_mode]
        ranking.append({'policy':policy['id'],'feasible':feasible,'expected_value':expected,'worst_case_value':worst,
